...
let teks =  `The rental time in this group has expired, the bot will exit automatically\nIf you want to rent again, please type .owner to find out the owner of the bot`
await conn.sendMessage(_dir[position].id, {text:`${teks}`})
await conn.groupLeave(_dir[position].id)
await conn.sendMessage(`94714201875@s.whatsapp.net`, {text: `Rental time in the group ${_dir[position].id} this is over`})
await _dir.splice(position, 1)
//await fs.writeFileSync('./database/sewa.json', JSON.stringify(_dir))
} else {
console.log('Delete unavailable rental data')
await _dir.splice(position, 1)
//await fs.writeFileSync('./database/sewa.json', JSON.stringify(_dir))
