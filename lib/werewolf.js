...
async function pagi(XeonBotInc, x, data) {
    ...
    return await XeonBotInc.sendMessage(x.room, {
        text: pagii(x),
        contextInfo: {
            externalAdReply: {
...
async function voting(XeonBotInc, x, data) {
    let row = [];
    let ment = [];
...
async function malam(XeonBotInc, x, data) {
        ...
        return XeonBotInc
            ...
            .then(() => {
                ...
                if (getWinner(x.room, data).status != null)
                    return win(x, 1, XeonBotInc, data);
            });
...
async function skill(XeonBotInc, x, data) {
    skillOn(x.room, data)
        ...
        for (let i = 0; i < x.player.length; i++) {
            if (x.player[i].role === "werewolf") {
                if (x.player[i].isdead != true) {
                    ...
                    await XeonBotInc.sendMessage(x.player[i].id, {
                        text: textnya,
                        mentions: membernya,
                    });
                }
...
async function win(x, t, XeonBotInc, data) {
    ...
    if (getWinner(x.room, data).status === false || x.iswin === false) {
        ...
        return await XeonBotInc.sendMessage(sesinya, {
            text: textnya,
            contextInfo: {
                externalAdReply: {
                    title: "W E R E W O L F",
...
// playing
async function run(XeonBotInc, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, XeonBotInc, data);
            break;
        } else {
            await sleep(90000);
        }
        if (getWinner(id, data).status != null) {
            ...
            await pagi(XeonBotInc, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, XeonBotInc, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await voting(XeonBotInc, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, XeonBotInc, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await malam(XeonBotInc, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, XeonBotInc, data);
            break;
        } else {
            await skill(XeonBotInc, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) break;
    }
    await win(sesi(id, data), 1, XeonBotInc, data);
}

async function run_vote(XeonBotInc, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, XeonBotInc, data);
            break;
...
async function run_malam(XeonBotInc, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, XeonBotInc, data);
            break;
...
async function run_pagi(XeonBotInc, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, XeonBotInc, data);
            break;
